...
import { parseProductsFromText } from "./productParser";

// This would be used to load products from the raw file in a real app
// For now, we'll use a subset of the data inline

export async function loadAllProducts(): Promise<Product[]> {
  try {
    // In a real implementation, you'd fetch the raw file
    // const response = await fetch('/data/products_raw.txt');
    // const text = await response.text();
    
    // For now, return parsed inline data
    return parseProductsFromText(getInlineProductData());
  } catch (error) {
...
function getInlineProductData(): string {
